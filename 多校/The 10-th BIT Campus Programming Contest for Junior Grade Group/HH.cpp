...
inline void upd_a(int x,int fl)
{
	...
	// else ok=0;
	cnt_a[x]+=fl;
	if(!cnt_a[x])sa.erase(x);
	else if(cnt_a[x]==1&&fl==1)sa.insert(x);
}
inline void upd_b(int x,int fl)
{
	...
	// else ok=0;
	cnt_b[x]+=fl;
	if(!cnt_b[x])sb.erase(x);
	else if(cnt_b[x]==1&&fl==1)sb.insert(x);
}
void solve()
{
	cin>>n>>m;
