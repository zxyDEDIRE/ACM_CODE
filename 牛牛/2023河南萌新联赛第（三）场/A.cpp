...
void pushdown(int rt)
{
	if(t[rt].lazy)
	{
		...
		t[rt<<1|1].sum+=t[rt].lazy*(t[rt<<1|1].r-t[rt<<1|1].l+1);
...
void update(int rt,int l,int r,int k,int ti)
{
	...
	{
		t[rt].sum+=t[rt].val*(t[rt].r-t[rt].l+1)*(ti-t[rt].t);
		t[rt].lazy+=t[rt].val*(ti-t[rt].t);
		t[rt].val=k;
		t[rt].t=ti;
		return ;
	}
...
void query(int rt,int l,int r,int ti)
{
	...
	{
		ans+=t[rt].sum;
		ans+=t[rt].val*(t[rt].r-t[rt].l+1)*(ti-t[rt].t);
		return ;
	}
	int mid=(t[rt].l+t[rt].r)>>1;
