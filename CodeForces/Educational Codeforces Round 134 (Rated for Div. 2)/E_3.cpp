...
struct KMP{
	...
	void init(char *s)
	{
		...
		for(int i=2;i<=len;i++)
		{
			...
			// 	if(nxt[i]<=i/2||s[nxt[nxt[i]]+1]==s[i])
			// 		nxt[i]=nxt[nxt[i]];
			// 	else {
			// 		// nxt[i]=nxt[nxt[i]];
			// 		nxt[i]=(i%(i-nxt[i])+i-nxt[i]);
			// 	}
			// 	// nxt[i]=nxt[nxt[i]];
			// }
			while(s[nxt[i]+1]!=s[i]&&nxt[i]){
