...
bool checkl(pii p1, pii p2, Vec& vis){
    auto [x1, y1] = p1; 
    auto [x2, y2] = p2;
    int flag = 1;
    for(int i = x1 + 1; i <= x2 && flag; i ++){
        if(i == x2 && y1 == y2) return true;
        if(!vis[get_id(i, y1)] && s[i][y1][0] != '-') flag = 0;
    }
    for(int j = y1 - 1; j >= y2 && flag; j --){
        if(j == y2) return true;
        if(!vis[get_id(x2, j)] && s[x2][j][0] != '-') flag = 0;
    }

    flag = 1;
    for(int j = y1 - 1; j >= y2 && flag; j --){
        if(x1 == x2 && j == y2) return true;
        if(!vis[get_id(x1, j)] && s[x1][j][0] != '-') flag = 0;
    }
    for(int i = x1 + 1; i <= x2 && flag; i ++){
        if(i == x2) return true;
        if(!vis[get_id(i, y2)] && s[i][y2][0] != '-') flag = 0;
    }
    return false;
}

bool checkr(pii p1, pii p2, Vec& vis){
    auto [x1, y1] = p1; 
    auto [x2, y2] = p2;
    int flag = 1;
    for(int i = x1 + 1; i <= x2 && flag; i ++){
...
bool check(int id1, int id2, Vec& vis){
	...
	if(y2 <= y1) return checkl({x1, y1}, {x2, y2}, vis);
    return checkr({x1, y1}, {x2, y2}, vis);
}
